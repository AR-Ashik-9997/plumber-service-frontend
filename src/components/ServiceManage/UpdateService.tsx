...
const UpdateService = ({ params }: IDProps) => {
  ...
  const onSubmit: SubmitHandler<IServices> = async (data: IServices) => { 
    ...
    await axios
      .patch(`https://plumber-service-one.vercel.app/api/v1/services/${id}`, formData, {
        headers: {
                  ...
                  <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark ">
                    ...
                    <div className="p-7">
                      {serviceData?.ServiceDetails?.map(
                        (item: IGetServiceDetails, index: number) => (
